...
static constexpr int FIRST_TURN_BUDGET_US = 700000; /* CG 1000ms hard: setup+print need headroom */
static constexpr int TURN_BUDGET_US = 50000; /* CG 75ms hard: leave ~25ms margin */
static constexpr int IBR_BUDGET_US = 500;
static constexpr int MIN_PHASE2_BUDGET_US = 30000;
static constexpr int MCTS_BUDGET_US = 0;
static constexpr int GA_POP_SIZE = 64;
static constexpr int OPP_POP_SIZE = 32;
static constexpr int GA_HORIZON = 8;
static constexpr double CROSSOVER_PROB = 0.25;
static constexpr double MUTATION_BASE_PROB = 0.25;
static constexpr bool BANG_BANG_THRUST = true;
...
static constexpr int N_WORKERS = 1; /* CG-safe single thread if 1 */ /* CG paste: never multi-thread on CodinGame */
static constexpr int EVAL_BATCH = 64;
inline void pin_worker(int worker_id) {
#if defined(__APPLE__)
thread_affinity_policy_data_t policy = { worker_id + 1 };
...
cpu_set_t cpuset;
CPU_ZERO(&cpuset);
CPU_SET(worker_id % N_WORKERS, &cpuset);
...
Ind* inds;
int n;
const csb::Game* base_game;
int H;
const double* EX; const double* EY;
const double* DTE; const double* RRX; const double* RRY;
int ncp, laps, mx, rp, bp, orp, obp, rb, fboost, rtimeout;
double dw, aw, sw, lw, apw, byw, opw, sfw, faw, rw;
const double* opp_ra; const int* opp_rt;
const double* opp_ba; const int* opp_bt;
std::atomic<int>* next;
std::atomic<int>* done;
};
inline void eval_worker_loop(int worker_id, std::atomic<bool>* stop,
std::atomic<int>* epoch, EvalJobCtx* job) {
pin_worker(worker_id);
int seen = 0;
while (!stop->load(std::memory_order_acquire)) {
int e = epoch->load(std::memory_order_acquire);
if (e == seen) {
for (int k = 0; k < 64 && epoch->load(std::memory_order_relaxed) == seen; k++) {}
...
EvalJobCtx j = *job;
for (;;) {
int i = j.next->fetch_add(1, std::memory_order_relaxed);
if (i >= j.n) break;
j.inds[i].sc = evaluate(&j.inds[i], *j.base_game, j.H, j.EX, j.EY, j.DTE, j.RRX, j.RRY,
j.ncp, j.laps, j.mx, j.rp, j.bp, j.orp, j.obp, j.rb, j.fboost, j.rtimeout,
j.dw, j.aw, j.sw, j.lw, j.apw, j.byw, j.opw, j.sfw, j.faw, j.rw,
j.opp_ra, j.opp_rt, j.opp_ba, j.opp_bt);
j.done->fetch_add(1, std::memory_order_acq_rel);
}
}
}
struct ParallelEvalPool {
...
workers[w] = std::thread(eval_worker_loop, w, &stop, &epoch, &job);
}
started = true;
}
void shutdown() {
...
for (int w = 0; w < N_WORKERS; w++) if (workers[w].joinable()) workers[w].join();
started = false;
}
void run(Ind* inds, int n,
...
if (n == 1 || N_WORKERS <= 1) {
for (int i = 0; i < n; i++) {
inds[i].sc = evaluate(&inds[i], base_game, H, EX, EY, DTE, RRX, RRY,
...
job.inds = inds; job.n = n; job.base_game = &base_game; job.H = H;
job.EX = EX; job.EY = EY; job.DTE = DTE; job.RRX = RRX; job.RRY = RRY;
job.ncp = ncp; job.laps = laps; job.mx = mx;
job.rp = rp; job.bp = bp; job.orp = orp; job.obp = obp; job.rb = rb;
job.fboost = fboost; job.rtimeout = rtimeout;
job.dw = dw; job.aw = aw; job.sw = sw; job.lw = lw; job.apw = apw;
job.byw = byw; job.opw = opw; job.sfw = sfw; job.faw = faw; job.rw = rw;
job.opp_ra = opp_ra; job.opp_rt = opp_rt; job.opp_ba = opp_ba; job.opp_bt = opp_bt;
job.next = &next; job.done = &done;
...
inline ParallelEvalPool& eval_pool() {
static ParallelEvalPool pool;
return pool;
}
inline void parallel_evaluate_batch(
Ind* inds, int n,
const csb::Game& base_game, int H,
const double* EX, const double* EY,
const double* DTE, const double* RRX, const double* RRY,
...
eval_pool().run(inds, n, base_game, H, EX, EY, DTE, RRX, RRY,
ncp, laps, mx, rp, bp, orp, obp, rb, fboost, rtimeout,
dw, aw, sw, lw, apw, byw, opw, sfw, faw, rw,
opp_ra, opp_rt, opp_ba, opp_bt);
}
...
while(1) {
iters++;if((iters&31)==0){clock_gettime(CLOCK_MONOTONIC,&tn);
...
parallel_evaluate_batch(pop, PS, base_game, H, EX, EY, DTE, RRX, RRY,
ncp, laps, mx, rp, bp, orp, obp, rb, fboost, rtimeout,
dw, aw, sw, lw, apw, byw, opw, sfw, faw, rw,
opp_ra, opp_rt, opp_ba, opp_bt);
int best = 0, worst = 0;
...
if (rng.rd() < 0.08) {
int t0m = rng.ri(0, H - 1), t1m = rng.ri(0, H - 1);
std::swap(child.ra[t0m], child.ba[t0m]);
std::swap(child.rt[t0m], child.bt[t0m]);
if (t1m != t0m) {
std::swap(child.ra[t1m], child.ba[t1m]);
std::swap(child.rt[t1m], child.bt[t1m]);
}
}
{ int si = rng.ri(0, H - 1); double v = child.ra[si] + rng.rd() * 24 - 12; child.ra[si] = v < -18 ? -18 : (v > 18 ? 18 : v); }
{ int si = rng.ri(0, H - 1); child.rt[si] = mutate_thrust(rng, child.rt[si]); }
...
parallel_evaluate_batch(batch, batch_n, base_game, H, EX, EY, DTE, RRX, RRY,
ncp, laps, mx, rp, bp, orp, obp, rb, fboost, rtimeout,
dw, aw, sw, lw, apw, byw, opw, sfw, faw, rw,
opp_ra, opp_rt, opp_ba, opp_bt);
iters += batch_n;
...
turn_++;
/* CG_TURN_T0: wall clock from first instruction of the turn */
struct timespec _cg_turn0; clock_gettime(CLOCK_MONOTONIC, &_cg_turn0);
...
/* CG_HARD_CAP: never schedule more than remaining hard limit from turn start */
/* budget clock shares turn0 */ struct timespec _cg_t0 = _cg_turn0;
const int CG_HARD_US = (turn_ == 1) ? 920000 : 65000;
...
{
struct timespec _cg_now; clock_gettime(CLOCK_MONOTONIC, &_cg_now);
long used = (_cg_now.tv_sec - _cg_turn0.tv_sec) * 1000000L + (_cg_now.tv_nsec - _cg_turn0.tv_nsec) / 1000L;
long rem = (long)CG_HARD_US - used - 2000; /* 2ms encode/print reserve */
if (rem < 5000) rem = 5000;
if ((long)budget_us > rem) budget_us = (int)rem;
}
bool use_ibr = false;
...
(ultimate_ga::SEARCH_MODE == 1) ? "CEM" : "GA8";
...
std::cout<<(int)a0.target_x<<" "<<(int)a0.target_y<<" "<<a0.thrust_str()<<"\n";
