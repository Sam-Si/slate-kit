// Magus SearchBot CodinGame paste. Do not hand-edit.
// Thinker: SEARCHBOT_CHALLENGER play() DEPTH=6 POP=48, emit-matched bang-bang.
// v2: activation timing + handling-by-avg-dist + tied-progress + next-CP 2.5. Eval vs frozen v1 on Agade rotate+jitter maps.
// Paste this whole file into the CodinGame C++ IDE (Mad Pod Racing / CSB).
...
static inline void sb_apply_opp_turn(int my_id) {
    ...
    sb_apply_policy(orun, cp->x - 3.f * orun->vx, cp->y - 3.f * orun->vy, MAX_THRUST);
...
public:
    ...
    void move(Solution* sol) {
#ifdef SEARCHBOT_CHALLENGER
        // Score the same racer bang-bang emit will play (heading-to-CP, 75° on
        // 5+ CP maps / 90° otherwise). Intermediate gene thrusts were scored
        // then overwritten at print_move.
        ...
        for (int pid = 0; pid < 2; pid++) {
            ...
            if (pid == racer && thrust != 650 && thrust != -1) {
...
#ifdef SEARCHBOT_CHALLENGER
    void seed_forward(Solution* s, int racer_pid) {
        for (int t = 0; t < DEPTH; t++) {
            for (int pid = 0; pid < 2; pid++) {
                ...
                if (pid == racer_pid) {
                    Checkpoint* cp = cps[pod->ncpid];
                    float raw = pod->diff_angle_xy(cp->x - 3.f * pod->vx,
                                                  cp->y - 3.f * pod->vy);
                    // Angle aims with 3*v lead; thrust matches emit (raw heading).
                    float heading = fabsf(pod->diff_angle(cps[pod->ncpid]));
                    thrust = heading < (cp_ct >= 5 ? 75.f : 90.f) ? MAX_THRUST : 0;
                    if (r == 0 && t == 0 && pod->dist(cps[1]) > 4000.f) thrust = 650;
    ...
    void solve(float time, bool with_seed = false) {
        ...
        seed_forward(&pop[n], 0);
        n++;
        seed_forward(&pop[n], 1);
        n++;
        while (n < POP) {
            pop[n] = pop[rnd(n)];
            pop[n].mutate();
        ...
        while (TIME < time) {
            ...
            pop[p].mutate(&child);
            if (rnd(5) == 0) {
                // Pin only the current racer step to bang-bang, not the
                // whole horizon — full-horizon 200 caused late braking.
                int racer = (sb_dte(pods[id]) <= sb_dte(pods[id + 1])) ? 0 : 1;
                int idx = racer * DEPTH;
                if (child.thrusts[idx] != 650 && child.thrusts[idx] != -1)
                    child.thrusts[idx] = MAX_THRUST;
                child.score = -1;
            } else if (rnd(3) == 0) {
                int idx = rnd(2 * DEPTH);
                if (child.thrusts[idx] != -1 && child.thrusts[idx] != 650)
                    child.thrusts[idx] = (child.thrusts[idx] < 100) ? MAX_THRUST : 0;
            }
    ...
    float evaluate() {
        ...
        Pod* mbk = mr->partner;
        Pod* orun = dc <= dd ? c : d;
        // Handling by avg CP spacing (Ultimate/Agade): short 5+ CP tracks
        // need more heading, less raw speed.
        float perim = 0.f;
        for (int i = 0; i < cp_ct; i++) perim += cps[i]->dist(cps[(i + 1) % cp_ct]);
        const bool handling = (cp_ct >= 5 && perim / (float)cp_ct <= 6500.f);
        const float dw = handling ? 2.9f : 2.5f;
        const float aw = handling ? 3.7f : 2.2f;
        const float sw = handling ? 0.20f : 0.45f;
        const float lw = handling ? 1.0f : 1.2f;
        const float apw = handling ? 60.f : 45.f;
        float score = 0.f;
        ...
        score -= fabsf(mr->diff_angle(cp)) * apw;
        score += sqrtf(mr->vx * mr->vx + mr->vy * mr->vy) * sw;
        if (cpd < 3000.f) {
            int nn = (mr->ncpid + 1) % cp_ct;
            float nnx = cps[nn]->x - mr->x, nny = cps[nn]->y - mr->y;
            float nnd = sqrtf(nnx * nnx + nny * nny);
            if (nnd > 1.f) score += (mr->vx * nnx + mr->vy * nny) / nnd * 2.5f;
        }
...
int main() {
    ...
    while (true) {
        ...
        const float tl = (r == 0) ? 0.90f : 0.074f;
        opp.solve(0.002f);
        ...
        auto emit = [&](Pod* pod, int thrust, float ang) {
            if (thrust == 650 && !pod->has_boost) thrust = MAX_THRUST;
            if (pod == racer && thrust != 650 && thrust != -1) {
                float heading = fabsf(pod->diff_angle(cps[pod->ncpid]));
                thrust = heading < bang ? MAX_THRUST : 0;
            }
