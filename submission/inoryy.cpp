...
// ── Official CodinGame CSB Physics Constants ───────────────────────
...
constexpr int GA_HORIZON = 6;
constexpr int GA_POP_SIZE = 80;
constexpr int OPP_POP_SIZE = 32;
constexpr int FIRST_TURN_BUDGET_US = 880000;
constexpr int TURN_BUDGET_US = 70000; // 74.5ms CSB budget matching top bots
constexpr int IBR_BUDGET_US = 2500;
constexpr int MCTS_BUDGET_US = 1200;
...
// ── Fast Look-up Table for Angle Deltas ────────────────────────────
...
inline SinCos fast_sincos(double x) {
    ...
    x -= k * TWO_PI;
    double x2 = x * x;
    double s = x * (1.0 + x2 * (-0.1666666664 + x2 * (0.0083333315 + x2 * (-0.0001984090 + x2 * 0.0000027526))));
    double c = 1.0 + x2 * (-0.5 + x2 * (0.0416666664 + x2 * (-0.0013888884 + x2 * (0.0000248010 - x2 * 0.0000002752))));
    return {s, c};
}
...
struct Vec2 {
    ...
    inline double dist(const Vec2& o) const { double dx = x - o.x, dy = y - o.y; return std::sqrt(dx * dx + dy * dy); }
    inline double dist2(const Vec2& o) const { double dx = x - o.x, dy = y - o.y; return dx * dx + dy * dy; }
};
...
struct Pod {
    ...
    inline void applyRotate(double target_angle) {
        ...
        if (da <= -MAX_ROTATE) angle -= MAX_ROTATE;
        else if (da >= MAX_ROTATE) angle += MAX_ROTATE;
    ...
    inline void passCheckpoint(int podn, int totalRaceCps, int* playerTimeout) {
        ...
        if (podn < 2) playerTimeout[0] = TIMEOUT_RESET;
        else playerTimeout[1] = TIMEOUT_RESET;
    }
};
...
struct alignas(64) SimGame {
    ...
    inline void setPodState(int idx, double x, double y, double vx, double vy,
        ...
        pod.won = false;
        pod.isFirstTurn = (std::abs(angle_rad + 0.0174533) < 0.001) || (std::abs(angle_rad) < 0.001);
    }

    inline void applyAction(int pod_idx, int target_x, int target_y, int thrust_val) {
        ...
        if (dx != 0.0 || dy != 0.0) {
            ...
            if (__builtin_expect(pod.isFirstTurn, 0)) { pod.isFirstTurn = false; pod.applyRotateFirst(target_angle); }
            else pod.applyRotate(target_angle);
        }
        int thrust = (pod.shieldtimer > 0 || (dx == 0.0 && dy == 0.0)) ? 0 : std::clamp(thrust_val, 0, MAX_THRUST);
        if (thrust > 0) pod.applyThrust(thrust);
    }

    inline void applyAction(int pod_idx, int target_x, int target_y, std::string_view thrust_str) {
        if (pod_idx < 0 || pod_idx >= POD_COUNT) return;
        Pod& pod = pods[pod_idx];
        Vec2 target = {static_cast<double>(target_x), static_cast<double>(target_y)};
        bool is_shield = false, is_boost = false;
        int val = 0;
        bool is_num = false;

        if (thrust_str == "200") { val = 200; is_num = true; }
        else if (thrust_str == "0") { val = 0; is_num = true; }
        else if (!thrust_str.empty()) {
            if (thrust_str[0] == 'S') is_shield = true;
            else if (thrust_str[0] == 'B') is_boost = true;
            else {
                is_num = true;
                ...
                for (; i < thrust_str.size(); ++i) {
                    if (thrust_str[i] >= '0' && thrust_str[i] <= '9') v = v * 10 + (thrust_str[i] - '0');
                    else break;
                }
                val = neg ? -v : v;
            }
        ...
        if (is_num) {
            if (val < 0) { pod.shieldtimer = SHIELD_ACTIVE_TIMER; if (pod.isFirstTurn) pod.isFirstTurn = false; return; }
            if (val > MAX_THRUST) { if (pod.isFirstTurn) pod.isFirstTurn = false; return; }
        }
        ...
        if (is_shield) { pod.shieldtimer = SHIELD_ACTIVE_TIMER; used_shield = true; thrust = 0; }
        else if (is_boost) {
            if (pod.boosted == 0) { thrust = BOOST_THRUST; pod.boosted = 1; }
    ...
    inline void bounce(int idx_a, int idx_b) {
        ...
        double m1 = (pa.shieldtimer == SHIELD_ACTIVE_TIMER) ? SHIELD_INV_MASS : NORMAL_INV_MASS;
        double m2 = (pb.shieldtimer == SHIELD_ACTIVE_TIMER) ? SHIELD_INV_MASS : NORMAL_INV_MASS;
        double force = normal.dot(relv) / (m1 + m2);
        if (force < MIN_IMPULSE) force += MIN_IMPULSE;
        else force += force;
    ...
    inline void nextTurn(int skip_rp = -1, int keep_bp = -1) {
        ...
        Vec2 curps[POD_COUNT] = {pods[0].pos, pods[1].pos, pods[2].pos, pods[3].pos};
        ...
        if (__builtin_expect(skip_rp < 0, 1)) {
            while (remaining > 0.0) {
                ...
                tx = pods[3].newCollide(pods[2], POD_RSQ, first); if (tx <= first) { first = tx; col_i = 3; col_j = 2; }
                tx = pods[3].newCollide(pods[1], POD_RSQ, first); if (tx <= first) { first = tx; col_i = 3; col_j = 1; }
                tx = pods[3].newCollide(pods[0], POD_RSQ, first); if (tx <= first) { first = tx; col_i = 3; col_j = 0; }
                tx = pods[2].newCollide(pods[1], POD_RSQ, first); if (tx <= first) { first = tx; col_i = 2; col_j = 1; }
                tx = pods[2].newCollide(pods[0], POD_RSQ, first); if (tx <= first) { first = tx; col_i = 2; col_j = 0; }
                tx = pods[1].newCollide(pods[0], POD_RSQ, first); if (tx <= first) { first = tx; col_i = 1; col_j = 0; }
                ...
                if (cpCollide(curps[col_i], pods[col_i].pos, globalCp[pods[col_i].next], CP_RSQ)) {
                    pods[col_i].passCheckpoint(col_i, totalRaceCps, playerTimeout);
                }
                if (cpCollide(curps[col_j], pods[col_j].pos, globalCp[pods[col_j].next], CP_RSQ)) {
            ...
            while (remaining > 0.0) {
                ...
                if (!ignore_pair(3, 2)) { tx = pods[3].newCollide(pods[2], POD_RSQ, first); if (tx <= first) { first = tx; col_i = 3; col_j = 2; } }
                if (!ignore_pair(3, 1)) { tx = pods[3].newCollide(pods[1], POD_RSQ, first); if (tx <= first) { first = tx; col_i = 3; col_j = 1; } }
                if (!ignore_pair(3, 0)) { tx = pods[3].newCollide(pods[0], POD_RSQ, first); if (tx <= first) { first = tx; col_i = 3; col_j = 0; } }
                if (!ignore_pair(2, 1)) { tx = pods[2].newCollide(pods[1], POD_RSQ, first); if (tx <= first) { first = tx; col_i = 2; col_j = 1; } }
                if (!ignore_pair(2, 0)) { tx = pods[2].newCollide(pods[0], POD_RSQ, first); if (tx <= first) { first = tx; col_i = 2; col_j = 0; } }
                if (!ignore_pair(1, 0)) { tx = pods[1].newCollide(pods[0], POD_RSQ, first); if (tx <= first) { first = tx; col_i = 1; col_j = 0; } }
        ...
        for (int i = 0; i < POD_COUNT; ++i) {
            ...
            if (cpCollide(curps[i], pods[i].pos, globalCp[pods[i].next], CP_RSQ)) {
                ...
                if (__builtin_expect(d_sq == CP_RSQ, 0)) pods[i].passCheckpoint(i, totalRaceCps, playerTimeout);
            }
        }
    }
};

// ── Fast Divisionless RNG ──────────────────────────────────────────
struct RNG {
    uint32_t s = 42;
    ...
    inline double rd() {
        return (u32() >> 8) * (1.0 / 16777216.0);
    }
    inline void seed(uint32_t v) { s = v; s64 = (static_cast<uint64_t>(v) << 32) ^ 88172645463325252ULL; }
};
...
// ── Opponent Perspective Evaluator ─────────────────────────────────
inline double evaluate_opp_perspective(
    const Ind* ind, const SimGame& base_game, const EvalContext& ctx,
    ...
    if (__builtin_expect(sim.pods[orp].pos.x < -2000.0 || sim.pods[orp].pos.x > 18000.0 ||
                         sim.pods[orp].pos.y < -2000.0 || sim.pods[orp].pos.y > 11000.0, 0)) return -1e9;

    double s = 0.0;
    if (__builtin_expect(sim.pods[orp].won, 0)) s += WIN_BONUS;
    if (__builtin_expect(sim.pods[rp].won, 0)) s -= WIN_BONUS;
...

    {
        ...
        if (d > 0.0) {
            double inv_d = 1.0 / d;
            s += (sim.pods[orp].vel.x * dx * inv_d + sim.pods[orp].vel.y * dy * inv_d) * ctx.aw;
        }
...

    {
        ...
        if (diff == 0) {
            ...
            s += (std::sqrt(our_dx * our_dx + our_dy * our_dy) - std::sqrt(opp_dx * opp_dx + opp_dy * opp_dy)) * 0.5;
        }
    }
    ...
    double bdx = sim.pods[obp].pos.x - sim.pods[rp].pos.x;
    double bdy = sim.pods[obp].pos.y - sim.pods[rp].pos.y;
    s -= 0.5 * std::sqrt(bdx * bdx + bdy * bdy);

    return s;
}

// ── Opponent Prediction GA (IBR) ──────────────────────────────────
inline void run_opp_prediction_ga(
    RNG& rng, const SimGame& base_game, const EvalContext& ctx,
    const double* our_ra, const int* our_rt,
    const double* our_ba, const int* our_bt,
    const double* opp_prev_ra, const int* opp_prev_rt,
    const double* opp_prev_ba, const int* opp_prev_bt,
    int has_opp_prev, int budget_us,
    ...
    if (has_opp_prev && opp_prev_ra && opp_prev_rt && opp_prev_ba && opp_prev_bt) {
        for (int t = 0; t < H - 1; t++) {
            pop[0].ra[t] = opp_prev_ra[t + 1]; pop[0].rt[t] = opp_prev_rt[t + 1];
            pop[0].ba[t] = opp_prev_ba[t + 1]; pop[0].bt[t] = opp_prev_bt[t + 1];
        }
        pop[0].ra[H - 1] = rng.rd() * 36.0 - 18.0; pop[0].rt[H - 1] = rand_thrust(rng);
        pop[0].ba[H - 1] = rng.rd() * 36.0 - 18.0; pop[0].bt[H - 1] = rand_thrust(rng);
        ...
        for (int pv = 0; pv < 3 && idx < PS; pv++) {
            ...
            for (int t = 0; t < H; t++) {
                ...
                pop[idx].ra[t] = raw_a * 0.1;
                pop[idx].rt[t] = rand_thrust(rng);
            }
            idx++;
        }
    }
    ...
    if (idx < PS) {
        for (int t = 0; t < H; t++) {
            pop[idx].ra[t] = 0; pop[idx].rt[t] = 200;
            pop[idx].ba[t] = 0; pop[idx].bt[t] = 200;
        }
    ...
    int best = 0, worst = 0;
    for (int i = 0; i < PS; i++) {
        pop[i].sc = evaluate_opp_perspective(&pop[i], base_game, ctx, our_ra, our_rt, our_ba, our_bt);
        scores[i] = pop[i].sc;
    ...
    while (true) {
        ...
        int p1 = static_cast<int>((static_cast<uint32_t>(r_par) * static_cast<uint64_t>(PS)) >> 32);
        int p2 = static_cast<int>(((r_par >> 32) * static_cast<uint64_t>(PS)) >> 32);
        ...
        child.sc = evaluate_opp_perspective(&child, base_game, ctx, our_ra, our_rt, our_ba, our_bt);
        if (child.sc > best_sc) {
            best_sc = child.sc;
            best = worst;
        }
        ...
        if (child.sc > wsc) {
            worst = 0; wsc = scores[0];
            #pragma GCC unroll 8
            for (int i = 1; i < PS; i++) {
                if (scores[i] < wsc) { worst = i; wsc = scores[i]; }
            }
        }
    }
...
// ── V40 Champion Fitness Evaluator (Fully Vectorized & Term-Optimized) ──
struct V40ChampionFitnessEvaluator {
    static inline double evaluate(const Ind* ind, const SimGame& base_game, const EvalContext& ctx) {
        constexpr int H = GA_HORIZON; (void)ctx.H;
        ...
        for (int t = 0; t < H; t++) {
            int rth = ind->rt[t];
            int rb_flag = (__builtin_expect(t == 0 && fboost, 0) && sim.pods[rp].shieldtimer == 0) ? 1 : 0;
            int rs_flag = (!rb_flag && __builtin_expect(t == ind->rs && ind->rs < 3, 0) && sim.pods[rp].shieldtimer == 0) ? 1 : 0;
            sim.pods[rp].applyActionDelta(ind->ra[t], rth, rs_flag, rb_flag);

            int bth = ind->bt[t];
            int bs_flag = (__builtin_expect(t == ind->bs && ind->bs < 3, 0) && sim.pods[bp].shieldtimer == 0) ? 1 : 0;
            sim.pods[bp].applyActionDelta(ind->ba[t], bth, bs_flag, 0);

            if (opp_ra) {
                sim.pods[orp].applyActionDelta(opp_ra[t], opp_rt[t], 0, 0);
            } else {
                sim.applyAction(orp, static_cast<int>(EX[sim.pods[orp].next % ncp]), static_cast<int>(EY[sim.pods[orp].next % ncp]), 200);
            }
            if (opp_ba) {
                sim.pods[obp].applyActionDelta(opp_ba[t], opp_bt[t], 0, 0);
            } else {
                int our_target_cp = sim.pods[rp].next % ncp;
                sim.applyAction(obp, static_cast<int>(EX[our_target_cp]), static_cast<int>(EY[our_target_cp]), 200);
            }
            ...
            if (__builtin_expect(r_act > static_cast<double>(H) && sim.pods[rp].next != ir_cp, 0)) r_act = static_cast<double>(t) + 0.5;
            if (__builtin_expect(o_act > static_cast<double>(H) && sim.pods[orp].next != ior_cp, 0)) o_act = static_cast<double>(t) + 0.5;
        ...
        double out_dist = 0.0;
        if (sim.pods[rp].pos.x < -1000.0) out_dist += (-1000.0 - sim.pods[rp].pos.x);
        else if (sim.pods[rp].pos.x > 17000.0) out_dist += (sim.pods[rp].pos.x - 17000.0);
        if (sim.pods[rp].pos.y < -1000.0) out_dist += (-1000.0 - sim.pods[rp].pos.y);
        else if (sim.pods[rp].pos.y > 10000.0) out_dist += (sim.pods[rp].pos.y - 10000.0);
        ...
        if (__builtin_expect(sim.pods[rp].pos.x < -1000.0 || sim.pods[rp].pos.x > 17000.0 ||
                             sim.pods[rp].pos.y < -1000.0 || sim.pods[rp].pos.y > 10000.0, 0)) s -= 100000.0;
        if (__builtin_expect(sim.pods[bp].pos.x < -1000.0 || sim.pods[bp].pos.x > 17000.0 ||
                             sim.pods[bp].pos.y < -1000.0 || sim.pods[bp].pos.y > 10000.0, 0)) s -= 100000.0;

        s -= friendly_penalty;

        if (__builtin_expect(sim.pods[rp].won, 0)) s += WIN_BONUS;
        if (__builtin_expect(!racing_only && sim.pods[orp].won, 0)) s -= WIN_BONUS;

        int crossed = sim.pods[rp].next - ir_cp;
        if (crossed > 0) s += crossed * CP_CROSS_BONUS - static_cast<double>(r_act) * 400.0;
            ...
            if (rl < mx) {
                ...
                if (!racing_only) {
                    ...
                    if (ob_d2 < 1440000.0 && ob_d2 > 1.0) {
                        ...
                        double cpd2 = (tx - cur_cpx) * (tx - cur_cpx) + (ty - cur_cpy) * (ty - cur_cpy);
                        if (cpd2 > 250000.0) {
        ...
        if (!racing_only) {
            ...
            if (diff == 0) {
                double opp_dx = sim.pods[orp].pos.x - sim.globalCp[sim.pods[orp].next].x;
                double opp_dy = sim.pods[orp].pos.y - sim.globalCp[sim.pods[orp].next].y;
                s += (std::sqrt(opp_dx * opp_dx + opp_dy * opp_dy) - std::sqrt(d2_to_cp)) * 0.5;
            }
        }

        // Near-CP anticipation alignment
        {
        ...
        // Racing line speed and alignment terms
        {
            int cur_cp = sim.pods[rp].next % ncp;
            double dx = dx_ex;
            double dy = dy_ex;
            ...
            if (d > 1e-6) {
                double inv_d = 1.0 / d; double nx = dx * inv_d, ny = dy * inv_d;
                s += (sim.pods[rp].vel.x * nx + sim.pods[rp].vel.y * ny) * aw;
                if (d < 3000.0) {
                    if (__builtin_expect(ctx.out_ux != nullptr, 1)) {
                        if (ctx.out_len[cur_cp] > 1.0) {
                            double exit_align = sim.pods[rp].vel.x * ctx.out_ux[cur_cp] + sim.pods[rp].vel.y * ctx.out_uy[cur_cp];
                            s += exit_align * 2.0;
                        }
                    } else {
                        ...
                        if (out_len > 1.0) {
                            ...
                            double exit_align = sim.pods[rp].vel.x * out_ux + sim.pods[rp].vel.y * out_uy;
                            s += exit_align * 2.0;
                        }
                    }
                }
            ...
            if (in_len < 2800.0) {
                ...
                if (in_len > 1.0 && out_len > 1.0) {
                    turn_deg = std::abs(fast_atan2(in_dx * out_dy - in_dy * out_dx, in_dx * out_dx + in_dy * out_dy)) * RAD_TO_DEG;
                }
                constexpr double INV_80 = 1.0 / 80.0;
                constexpr double INV_2800 = 1.0 / 2800.0;
                double corner_tightness = std::clamp((turn_deg - 60.0) * INV_80, 0.0, 1.0);
                double corner_prox = (2800.0 - in_len) * INV_2800;
...

        {
            ...
            if (d2 < threshold2) {
                double penalty = byw * 20.0 * (1.0 - d2 * inv_threshold2);
        ...
        double opp_to_cp_d = std::sqrt(opp_to_cp_x * opp_to_cp_x + opp_to_cp_y * opp_to_cp_y);
        ...
        if (rtimeout == 1) {
            ...
            {
                double cx = sim.globalCp[target_rb].x - sim.pods[orp].pos.x, cy = sim.globalCp[target_rb].y - sim.pods[orp].pos.y;
            ...
            {
                double dx = sim.pods[bp].pos.x - RRX[target_rb_mod], dy = sim.pods[bp].pos.y - RRY[target_rb_mod];
                double d = std::sqrt(dx * dx + dy * dy);
            ...
            }
            {
                double dist = b_dist;
                if (dist < 1200.0) {
                    // RoboStac Pinning: relative velocity damping + stationary anchor
                ...
                if (dist < BLOCKER_PROX_FAR) {
                    constexpr double INV_FAR_CLOSE = 1.0 / (BLOCKER_PROX_FAR - BLOCKER_PROX_CLOSE);
                    constexpr double INV_MED_CLOSE = 1.0 / (BLOCKER_PROX_MED - BLOCKER_PROX_CLOSE);
            ...
            {
                double opp_init_speed = __builtin_expect(ctx.opp_init_speed > 0.0, 1) ? ctx.opp_init_speed : base_game.pods[orp].vel.norm();
...
// ── MCTS First-Move Candidate Probe ───────────────────────────────
...
inline int mcts_first_move_probe(
    ...
    for (int i = 0; i < MCTS_TOP_K; i++) {
        int best_j = i;
        double best_avg = sum_score[ranked[i]] / std::max(1, visit_count[ranked[i]]);
...
// ── Master Decoupled Genetic Optimizer ─────────────────────────────
template <typename FitnessEvaluator>
struct GeneticOptimizer {
    static int run(
        RNG& rng, const SimGame& base_game, const EvalContext& ctx, int budget_us,
        ...
        const Ind* lock_blocker = nullptr,
        int count_setup = 0
    ) {
        ...
        // 1. Warm start seed from previous turn
        if (has_prev && prev_ra && prev_rt && prev_ba && prev_bt) {
            ...
            // Mutated variations of warm start
        ...
        // 2. Heuristic Simulation Rollout Seeds
        {
            SimGame hsim = base_game;
            for (int t = 0; t < H; t++) {
                int cur_cp = hsim.pods[ctx.rp].next % ctx.ncp;
                double tx = ctx.EX[cur_cp], ty = ctx.EY[cur_cp];
                double ddx = tx - hsim.pods[ctx.rp].pos.x, ddy = ty - hsim.pods[ctx.rp].pos.y;
                double dd = std::sqrt(ddx * ddx + ddy * ddy);
                if (dd > 0) {
                    double ux = ddx / dd, uy = ddy / dd;
                    double perp = hsim.pods[ctx.rp].vel.x * (-uy) + hsim.pods[ctx.rp].vel.y * ux;
                ...
                double ta = fast_atan2(ty - hsim.pods[ctx.rp].pos.y, tx - hsim.pods[ctx.rp].pos.x);
                ...
                double bta = fast_atan2(bty - hsim.pods[ctx.bp].pos.y, btx - hsim.pods[ctx.bp].pos.x);
                ...
                hsim.applyAction(ctx.orp, static_cast<int>(ctx.EX[hsim.pods[ctx.orp].next % ctx.ncp]), static_cast<int>(ctx.EY[hsim.pods[ctx.orp].next % ctx.ncp]), 200);
                hsim.applyAction(ctx.obp, static_cast<int>(hsim.pods[ctx.rp].pos.x), static_cast<int>(hsim.pods[ctx.rp].pos.y), 200);
                hsim.nextTurn();
            }
        ...
        // 3. Intercept Heuristic Simulation Rollout
        if (idx + 1 < PS) {
            SimGame isim = base_game;
            for (int t = 0; t < H; t++) {
                int cur_cp = isim.pods[ctx.rp].next % ctx.ncp;
                double itx = ctx.EX[cur_cp], ity = ctx.EY[cur_cp];
                double ita = fast_atan2(ity - isim.pods[ctx.rp].pos.y, itx - isim.pods[ctx.rp].pos.x);
                ...
                double pred_x = isim.pods[ctx.orp].pos.x + isim.pods[ctx.orp].vel.x * 3.0;
                double pred_y = isim.pods[ctx.orp].pos.y + isim.pods[ctx.orp].vel.y * 3.0;
                double ibta = fast_atan2(pred_y - isim.pods[ctx.bp].pos.y, pred_x - isim.pods[ctx.bp].pos.x);
                ...
                isim.applyAction(ctx.orp, static_cast<int>(ctx.EX[isim.pods[ctx.orp].next % ctx.ncp]), static_cast<int>(ctx.EY[isim.pods[ctx.orp].next % ctx.ncp]), 200);
                isim.applyAction(ctx.obp, static_cast<int>(isim.pods[ctx.rp].pos.x), static_cast<int>(isim.pods[ctx.rp].pos.y), 200);
                isim.nextTurn();
            }
            pop[idx].rs = H; pop[idx].bs = H; idx++;
        }

        // 4. MCTS Discrete First-Move Probe Seeds
        {
            Ind mcts_seeds[MCTS_TOP_K];
            int n_mcts = mcts_first_move_probe(rng, base_game, ctx, MCTS_BUDGET_US, mcts_seeds);
        ...
        if (idx + 4 <= PS) {
            ...
            for (int k = 0; k < 4; k++) { pop[idx + k].rs = H; pop[idx + k].bs = H; }
            idx += 4;
        }

        // 7. Fill remainder of population with uniform exploration
        for (int i = idx; i < PS; i++) {
            for (int t = 0; t < H; t++) {
        ...
        struct timespec t0, tn;
        clock_gettime(CLOCK_MONOTONIC, &t0);
        if (count_setup) t0 = t_entry;
        int iters = PS;
        double amplitude = 1.0;
        uint32_t thresh_u32 = static_cast<uint32_t>((0.25 + amplitude) * 16777216.0);
        Ind child;

        // 9. Main Genetic Search with Simulated Annealing
        while (true) {
            iters++;
            if (__builtin_expect((iters & 63) == 0, 0)) {
                clock_gettime(CLOCK_MONOTONIC, &tn);
                long el = (tn.tv_sec - t0.tv_sec) * 1000000L + (tn.tv_nsec - t0.tv_nsec) / 1000L;
                if (el >= budget_us) break;
                amplitude = 1.0 - static_cast<double>(el) / static_cast<double>(budget_us);
            ...
            if (rng.u32() < 858993460U) {
                ...
                int p3 = static_cast<int>((static_cast<uint32_t>(r_par2) * static_cast<uint64_t>(PS)) >> 32);
                int p4 = static_cast<int>(((r_par2 >> 32) * static_cast<uint64_t>(PS)) >> 32);
                ...
                child.rs = (mask & 0x1000) ? pop[par].rs : pop[par2].rs;
                child.bs = (mask & 0x2000) ? pop[par].bs : pop[par2].bs;
            } else {
                child.copyFrom(pop[par]);
                for (int t = 0; t < H; t++) {
                    ...
                    if ((rng.u32() >> 8) < thresh_u32) { child.rt[t] = rand_thrust(rng); }
                    if ((rng.u32() >> 8) < thresh_u32) {
                        int raw = rng.ri(-400, 400);
                        if (raw < -180) raw = -180;
                        else if (raw > 180) raw = 180;
                    ...
                    if ((rng.u32() >> 8) < thresh_u32) { child.bt[t] = rand_thrust(rng); }
                }
...
public:
    ...
    void init(int laps, const std::vector<std::pair<double, double>>& checkpoints) {
        laps_ = laps + 1;
    ...
    std::pair<ActionOutput, ActionOutput> get_actions(
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (pang[i] < 0) {
                pang[i] = fast_atan2(CY_[pnc[i]] - py[i], CX_[pnc[i]] - px[i]) * inoryy_ga::RAD_TO_DEG;
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (pang[i + 2] < 0) {
                pang[i + 2] = fast_atan2(CY_[pnc[i + 2]] - py[i + 2], CX_[pnc[i + 2]] - px[i + 2]) * inoryy_ga::RAD_TO_DEG;
        ...
        int rb = pnc[orp_abs];
        {
            double od = std::hypot(px[orp_abs] - CX_[rb], py[orp_abs] - CY_[rb]);
            double md = std::hypot(px[bp] - CX_[rb], py[bp] - CY_[rb]);
            if (md > od + 1200.0 || (od < 1800.0 && md > od + 300.0)) {
                int nb = (rb + 1) % ncp_;
                double od2 = od + std::hypot(CX_[rb] - CX_[nb], CY_[rb] - CY_[nb]);
                double md2 = std::hypot(px[bp] - CX_[nb], py[bp] - CY_[nb]);
                if (md2 < od2 - 800.0) rb = nb;
                else rb = (nb + 1) % ncp_;
            }
        }
        ...
        if (!boost_used_[rp] && shield_cooldown_[rp] == 0) {
            if (turn_ == 1) {
                double d_start_cp = std::hypot(px[rp] - CX_[pnc[rp]], py[rp] - CY_[pnc[rp]]);
                if (d_start_cp > 3800.0) fboost = 1;
            } else {
                double dd = std::hypot(px[rp] - EX_[pnc[rp]], py[rp] - EY_[pnc[rp]]);
                double ta = fast_atan2(EY_[pnc[rp]] - py[rp], EX_[pnc[rp]] - px[rp]) * inoryy_ga::RAD_TO_DEG;
                if (ta < 0) ta += 360.0;
        ...
        for (int i = 0; i < 4; i++) {
            ...
            base_game.setPodState(i, px[i], py[i], pvx[i], pvy[i],
                                  ...
                                  (i < 2 ? shield_cooldown_[i] : 0), (i < 2 && boost_used_[i]) ? 1 : 0);
            base_game.pods[i].isFirstTurn = (turn_ == 1);
        }

        int total_budget_us = (turn_ == 1) ? inoryy_ga::FIRST_TURN_BUDGET_US : inoryy_ga::TURN_BUDGET_US;
        bool use_ibr = has_prev_ && turn_ > 1;
        int ibr_budget = use_ibr ? inoryy_ga::IBR_BUDGET_US : 0;

        auto t_now = std::chrono::steady_clock::now();
        int elapsed_prep_us = static_cast<int>(std::chrono::duration_cast<std::chrono::microseconds>(t_now - t_turn_start).count());
        int remaining_budget_us = total_budget_us - elapsed_prep_us;
        if (remaining_budget_us < 10000) remaining_budget_us = 10000;

        int main_budget = remaining_budget_us - ibr_budget;
        if (main_budget < 8000) main_budget = 8000;
        ...
        ctx.dw = dw_; ctx.aw = aw_; ctx.sw = sw_; ctx.lw = lw_; ctx.apw = apw_;
        ctx.byw = byw_; ctx.opw = opw_; ctx.sfw = sfw_; ctx.faw = faw_; ctx.rw = rw_;
        ...
        if (use_ibr) {
            double shifted_ra[inoryy_ga::GA_HORIZON], shifted_ba[inoryy_ga::GA_HORIZON];
            ...
            shifted_ra[inoryy_ga::GA_HORIZON - 1] = 0; shifted_rt[inoryy_ga::GA_HORIZON - 1] = 200;
            shifted_ba[inoryy_ga::GA_HORIZON - 1] = 0; shifted_bt[inoryy_ga::GA_HORIZON - 1] = 200;
            ...
            std::memcpy(opp_prev_ra_, pred_ora, inoryy_ga::GA_HORIZON * sizeof(double));
            std::memcpy(opp_prev_rt_, pred_ort, inoryy_ga::GA_HORIZON * sizeof(int));
            std::memcpy(opp_prev_ba_, pred_oba, inoryy_ga::GA_HORIZON * sizeof(double));
        ...
        // 2. Main GA Search (Fully Vectorized AVX-256 Engine)
        ...
        int ga_sims = inoryy_ga::GeneticOptimizer<inoryy_ga::V40ChampionFitnessEvaluator>::run(
            rng_, base_game, ctx, main_budget,
        ...
        auto is_way_off = [](double x, double y) {
            return x < -1200.0 || x > 17200.0 || y < -1200.0 || y > 10200.0;
        };
        for (int pod : {rp, bp}) {
            if (is_way_off(px[pod], py[pod])) {
                double tx = (pod == bp) ? CX_[rb] : CX_[pnc[pod]];
                double ty = (pod == bp) ? CY_[rb] : CY_[pnc[pod]];
                double target_angle = fast_atan2(ty - py[pod], tx - px[pod]) * inoryy_ga::RAD_TO_DEG;
                if (target_angle < 0.0) target_angle += 360.0;
                double angle_diff = target_angle - pang[pod];
                while (angle_diff > 180.0) angle_diff -= 360.0;
                while (angle_diff < -180.0) angle_diff += 360.0;
                ActionOutput recovery;
                recovery.target_x = std::lround(tx);
                recovery.target_y = std::lround(ty);
                recovery.thrust = (std::abs(angle_diff) > 75.0) ? 0 : 200;
                int other = (pod == rp) ? bp : rp;
                double* oa = (other == rp) ? out_ra : out_ba;
                int* ot = (other == rp) ? out_rt : out_bt;
                double da = std::clamp(oa[0], -18.0, 18.0);
                double ad = std::fmod(pang[other] + da, 360.0);
                if (ad < 0) ad += 360.0;
                double rd = ad * inoryy_ga::DEG_TO_RAD;
                ActionOutput normal;
                normal.target_x = std::lround(px[other] + std::cos(rd) * 10000.0);
                normal.target_y = std::lround(py[other] + std::sin(rd) * 10000.0);
                normal.thrust = std::clamp(ot[0], 0, 200);
                if (pod == 0) return {recovery, normal};
        ...
        if (r_shield) {
            ...
            act_runner.thrust = std::clamp(out_rt[0], 0, 200);
        }
        ...
        if (!b_shield && shield_cooldown_[bp] == 0) {
            double tc = base_game.pods[bp].newCollide(base_game.pods[orp_abs], inoryy_ga::POD_RSQ, 1.0);
            if (tc >= 0.0 && tc <= 1.0) {
                ...
                if (closing > 0.0 && std::hypot(dvx, dvy) > 200.0) {
                    b_shield = true;
                }
            }
        }
        ...
        if (!boost_used_[bp] && !b_shield && shield_cooldown_[bp] == 0) {
            ...
            if (dd > 5000.0) {
                double ba_t = fast_atan2(py[orp_abs] - py[bp], px[orp_abs] - px[bp]) * inoryy_ga::RAD_TO_DEG;
        ...
        if (b_shield) {
            ...
            int b_thr = std::clamp(out_bt[0], 0, 200);
            if (turn_ <= 5) {
                double b_dx_rb = CX_[rb] - px[bp], b_dy_rb = CY_[rb] - py[bp];
...
// Boundary guard removed - natural GA fitness boundary penalty active
        ...
        std::fprintf(stderr, "T%d 74.5ms GA sims:%d\n", turn_, ga_sims);

        if (rp == 0) return {act_runner, act_blocker};
        else return {act_blocker, act_runner};
    }
...
private:
    void compute_entry_points() {
        ...
        for (int i = 0; i < ncp_; i++) {
            ...
            if (l1 > 0 && l2 > 0) cos_theta = std::max(-1.0, std::min(1.0, (v1x * v2x + v1y * v2y) / (l1 * l2)));
            ...
            double dx = CX_[p] - CX_[n], dy = CY_[p] - CY_[n], dd = std::hypot(dx, dy);
            if (dd > 0) {
    ...
    void compute_weights() {
        double total_dist = 0;
        for (int i = 0; i < ncp_; i++) {
            total_dist += std::hypot(CX_[i] - CX_[(i + 1) % ncp_], CY_[i] - CY_[(i + 1) % ncp_]);
        }
...
// ── CodinGame Main Interactive Loop ────────────────────────────────
int main() {
    ...
    while (true) {
        ...
        std::cout << act0.target_x << " " << act0.target_y << " " << act0.thrust_str() << "\n";
        std::cout << act1.target_x << " " << act1.target_y << " " << act1.thrust_str() << "\n";
        std::cout.flush();
    }
    return 0;
}
