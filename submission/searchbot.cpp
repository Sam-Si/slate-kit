// Fenrir-emit SearchBot CodinGame paste. Do not hand-edit.
// Thinker: SEARCHBOT_CHALLENGER + SEARCHBOT_FENRIR, DEPTH=6 POP=48, T0=0.94s, later=0.074s.
// Official generate 13x2 seed 42 1000/75 vs unread frozen Magus B:
// 26g 24-2 wr=0.923. T0 does not force BOOST. Scored T0 BOOST if first-leg
// 5000-8000, heading<25, next corner<120 (Fenrir 10/43 were 5158-7593).
// Later aligned racer BOOST. Contact SHIELD when opp <900 and closing.
#define SEARCHBOT_CHALLENGER
#define SEARCHBOT_FENRIR
#pragma once

// Observable Fenrir emit (agent 6500255, 43 public replays).
// Used on the SEARCHBOT_FENRIR emit path. T0 never forces BOOST (33/43 were 200/200).
// Later racer BOOST: heading median ~17.5°, dist min ~714.
// Contact SHIELD stays at 900 (1200 gate was 21–5 wr=0.808 vs locked 0.923).

inline int sb_fenrir_bang_thrust(int turn, bool has_boost, float heading_deg,
                                 float dist_cp, int left_cps, bool even_or_ahead,
                                 int fallback_th) {
    if (turn <= 0) return fallback_th;
    if (!has_boost) return fallback_th;
    const float ah = heading_deg < 0.f ? -heading_deg : heading_deg;
    if (ah < 20.f && dist_cp > 700.f) return 650;
    if (left_cps <= 1 && left_cps >= 0 && ah < 25.f && even_or_ahead) return 650;
    return fallback_th;
}

inline bool sb_fenrir_contact_shield(float dmin, float closing_dot, int shield) {
    return shield <= 0 && dmin < 900.f && closing_dot > 0.f;
}

// Fenrir T0-BOOSTED 10/43, first-leg 5158–7593, never both pods, never >8k.
// Candidate only — kept iff get_score improves.
// bang_thrust() still returns the non-BOOST fallback on turn 0 (no force).
inline bool sb_fenrir_t0_boost_candidate(float dist_first, float heading_deg,
                                        float next_corner_deg) {
    const float ah = heading_deg < 0.f ? -heading_deg : heading_deg;
    return dist_first > 5000.f && dist_first < 8000.f && ah < 25.f
           && next_corner_deg < 120.f;
}

// Fenrir A2C 6-action alphabet (forum.codingame.com/t/neural-network-ressources/1667
// post 38): {200,−18},{200,0},{200,+18},{0,−18},{0,0},{0,+18} plus rare BOOST/SHIELD.
// SEARCHBOT_FENRIR randomize() samples this instead of 50/100/150 mid-thrust.
// Snap mid-thrust onto Fenrir's 0/200/SHIELD/BOOST alphabet. Angles stay Magus.
inline int sb_fenrir_snap_thrust(int th) {
    if (th == -1 || th == 650) return th;
    return th >= 100 ? 200 : 0;
}

inline void sb_fenrir_sample_gene(int angle_slot, int thrust_roll, bool shield,
                                 float* ang, int* th) {
    *ang = (angle_slot <= 0) ? -18.f : (angle_slot == 1 ? 0.f : 18.f);
    if (shield) {
        *th = -1;
    } else if (thrust_roll == 0) {
        *th = 650;
    } else if (thrust_roll < 14) {
        *th = 200;
    } else {
        *th = 0;
    }
        ...
        if (full || r == 1) {
            if (rnd(100) >= SHIELD_PROB) {
                ...
                if (roll == 0 && r == 0)
                    ...
                    thrusts[idx] = 0;
                // SEARCHBOT_FENRIR snap-in-randomize (50→0, ≥100→200) was
                // 21–5 wr=0.808 vs locked Fenrir-emit 0.923. Leave Magus
                // mid-thrust in search; Fenrir 0/200 lives on emit bang.
#else
...
#ifdef SEARCHBOT_CHALLENGER
// Fidelity Game rollout (latest referee physics). Declaration only in this paste.
...
// Heading change at CP1 (not dump_geo interior). Replay 900171716 /
// m11: 4215 @ 121°. Length cap is required: angle-only >=115 fires
// on 51/90 official maps (m0 5068@136) and t0hp went 26g wr=0.58.
// t0hld fl<4800 && heading>=115: 152g wr=0.816, did not convert m11.
// t0tiny added tiny-ring: converted m11 6-0 but that binary wrecked
// m2/m16 (hold does not fire there — dirty tree). Tight gate below
// hits only m11/m50 among official 90 (m7 fl=3722 already <4000).
static inline bool sb_t0_hold_boost(float first_leg) {
    return first_leg < 4800.f && sb_first_corner_deg() >= 115.f;
}

// t0hh 160g wr=0.831: this gate made m11 4-2 vs pf 5-1. Unused.
static inline bool sb_t0_hairpin_hold() {
    if (cp_ct < 4) return false;
    float fl = cps[0]->dist(cps[1]);
    return fl < 4400.f && sb_first_corner_deg() >= 120.f;
}
...
public:
    ...
    void solve(float time, bool with_seed = false) {
...
#ifdef SEARCHBOT_CHALLENGER
        sb_update_racer(id);
        // p32t POP=32 non-tiny / 48 tiny: ~108g wr=0.75. pop32 all-maps
        // 0.774. Stay POP=48 (pf 0.849).
        const int POP = 48;
        Solution pop[POP];
        int n = 0;
        if (with_seed) {
        ...
        while (TIME < time) {
            ...
            // pfx 25% xo on photo-finish: 144g wr=0.826, m8 stayed 3-3.
            // Stay pf 20% (rnd(5)).
            if (rnd(5) == 0) {
                int q = rnd(POP);
                child = pop[p];
            ...
            pop[p].mutate(&child);
            // ith rnd(3) 33% intermediate: 166g 138-28 wr=0.831.
            // Converted m8/m13/m19 but taxed m2/m16/m26. Stay 25%.
            if (rnd(4) == 0) {
                int idx = rnd(2 * DEPTH);
    ...
    // Exact-physics race only (checked + DTE). fid3 reranked Magus
    // genomes with Magus evaluate() on Fidelity states: 118g wr=0.746.
    // Emit pick of bang vs GA is two first-turn actions; score the
    // referee 6-step race, not Magus evaluate.
    float fid_race_score(Solution* s) {
        SearchBot* sb_opp = NULL;
        if (!oppBots.empty()) sb_opp = dynamic_cast<SearchBot*>(oppBots[0]);
        const int* oth = NULL;
        const float* oang = NULL;
        ...
        int start_ck = pods[id]->checked > pods[id + 1]->checked
                           ? pods[id]->checked : pods[id + 1]->checked;
        int start_opp = pods[(id + 2) % 4]->checked > pods[(id + 3) % 4]->checked
                            ? pods[(id + 2) % 4]->checked : pods[(id + 3) % 4]->checked;
        float r_act = (float)DEPTH + 0.3f;
        float o_act = (float)DEPTH + 0.3f;
        sb_fid_rollout(s->thrusts, s->angles, oth, oang, id, have,
                       false, opp_fr, &r_act, &o_act, start_ck, start_opp);
        float da = sb_dte(pods[id]), db = sb_dte(pods[id + 1]);
        float dc = sb_dte(pods[(id + 2) % 4]), dd = sb_dte(pods[(id + 3) % 4]);
        int mc = pods[id]->checked > pods[id + 1]->checked
                     ? pods[id]->checked : pods[id + 1]->checked;
        int oc = pods[(id + 2) % 4]->checked > pods[(id + 3) % 4]->checked
                     ? pods[(id + 2) % 4]->checked : pods[(id + 3) % 4]->checked;
        float score = 1000000.f * (float)(mc - oc) - (da <= db ? da : db)
                      + 0.35f * (dc <= dd ? dc : dd);
        if (mc >= laps * cp_ct) score += 1e9f;
        if (oc >= laps * cp_ct) score -= 1e9f;
        load();
        return score;
    }
#endif
    ...
    float evaluate() {
        ...
        // m1 2-4 / m11 3-3. l1g last-1 4+CP non-tiny: 0.797 taxed m4.
        // l1p last-1 pf heading: 96g wr=0.78, m8 5-1 but taxed m1 4-2.
        // Official best is DTE (pf 0.849).
        Pod* mr = da <= db ? a : b;
        Pod* orun = dc <= dd ? c : d;
        Pod* mbk = mr->partner;
        ...
        // l1d 0.8 last-1 DTE: 148g 120-28 wr=0.811. l1dg gated 0.3:
        // 167g 139-28 wr=0.832; m2 3-3 vs pf 6-0. Revert to pf DTE.
        ...
        // ck18 151g wr=0.815 / ck18c live m0 3-3 vs pf 5-1: stay 15000.
        ...
        // pfl1 last-CP blocker: 141g wr=0.794. Converted m8/m19 but
        // wrecked m10 2-4. Back to pf 1200/800.
        if (!trail && bd < 5000.f) {
...
#endif
// CG I/O for Magus SearchBot (play()/search).
int main() {
    ...
    while (true) {
        ...
        if (cp_ct >= 4) {
            ...
            Pod* rp_pod = pods[rp];
            {
...
#ifdef SEARCHBOT_FENRIR
                {
                    ...
                    th = sb_fenrir_bang_thrust(r, rp_pod->has_boost, raw,
                                               rp_pod->dist(cp), left,
                                               myck >= opck, th);
                }
#else
...
#ifdef SEARCHBOT_FENRIR
                if (r == 0 && rp_pod->has_boost) {
                    int nn = (rp_pod->ncpid + 1) % cp_ct;
                    float ix = cp->x - rp_pod->x, iy = cp->y - rp_pod->y;
                    float ox = cps[nn]->x - cp->x, oy = cps[nn]->y - cp->y;
                    float inn = sqrtf(ix * ix + iy * iy), onn = sqrtf(ox * ox + oy * oy);
                    float corner = 0.f;
                    if (inn > 1.f && onn > 1.f)
                        corner = fabsf(atan2f(ix * oy - iy * ox, ix * ox + iy * oy))
                                 * (180.f / 3.14159265f);
                    if (sb_fenrir_t0_boost_candidate(rp_pod->dist(cp), raw, corner)) {
                        Solution t0b = me.sol;
                        t0b.score = -1;
                        t0b.angles[idx] = max(-18.f, min(18.f, raw));
                        t0b.thrusts[idx] = 650;
                        if (me.get_score(&t0b) > me.get_score(&me.sol)) me.sol = t0b;
                    }
                }
#endif
            }
        ...
        for (int pi = 0; pi < 2; pi++) {
            ...
            for (int oi = 2; oi < 4; oi++) {
                float dx = mep->x - pods[oi]->x, dy = mep->y - pods[oi]->y;
                float d = sqrtf(dx * dx + dy * dy);
                if (d < best) {
                    best = d;
                    near = pods[oi];
            ...
            if (!near) continue;
            float rdx = mep->x - near->x, rdy = mep->y - near->y;
            float rvx = near->vx - mep->vx, rvy = near->vy - mep->vy;
            float closing = rvx * rdx + rvy * rdy;
            if (!sb_fenrir_contact_shield(best, closing, mep->shield)) continue;
            Solution sh = me.sol;
            sh.score = -1;
            sh.thrusts[pi * DEPTH] = -1;
            float ga_s = me.get_score(&me.sol);
            float sh_s = me.get_score(&sh);
            if (sh_s > ga_s) me.sol = sh;
        }
#endif
