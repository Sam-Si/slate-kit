// GENERATED Fenrir-emit SearchBot CodinGame paste. Do not hand-edit.
// Sources: fenrir_emit.h + opt_search.inc + cg_paste_main.cpp
...
// 26g 24-1-1 wr=0.923. T0 does not force BOOST. Later aligned racer BOOST.
// Contact SHIELD when opp <900 and closing. Paste into the CG C++ IDE.
#define SEARCHBOT_CHALLENGER
#define SEARCHBOT_FENRIR
// Observable Fenrir emit (agent 6500255, 22 public replays via
// sim.battle_parser.load_battle). challenger.cpp calls these on the
// SEARCHBOT_FENRIR emit path. T0 never forces BOOST (17/22 were 200/200).
// Later racer BOOST: 14 events, heading median ~15°, dist min ~714.
// Contact SHIELD: 232 events, 58 with dmin<900.
...
// Fenrir T0-BOOSTED 5/22 (first-leg 5158–7593), never both pods. This is a
// *candidate* only — challenger.cpp must keep it iff get_score improves.
...
inline bool sb_fenrir_t0_boost_candidate(float dist_first, float heading_deg,
    ...
    const float ah = heading_deg < 0.f ? -heading_deg : heading_deg;
    // m5 seed42 fl=8936 hc=91 → yes; m8 fl=12624 hc=152 → no (BOOST into hairpin).
    return dist_first > 8000.f && ah < 25.f && next_corner_deg < 120.f;
}
