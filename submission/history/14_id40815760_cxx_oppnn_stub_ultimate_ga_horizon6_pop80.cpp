...
static constexpr int TURN_BUDGET_US = 74000;
static constexpr int IBR_BUDGET_US = 15000;
...
sim.applyAction(orp, (int)EX[sim.pods[orp].next % ncp], (int)EY[sim.pods[orp].next % ncp], "200");
...
double dd = sqrt((sim.pods[obp].pos.x - sim.pods[rp].pos.x) * (sim.pods[obp].pos.x - sim.pods[rp].pos.x) +
(sim.pods[obp].pos.y - sim.pods[rp].pos.y) * (sim.pods[obp].pos.y - sim.pods[rp].pos.y));
int sh = (dd < 850 && sim.pods[obp].shieldtimer == 0) ? 1 : 0;
if (sh) { sim.applyAction(obp, (int)tx2, (int)ty2, "SHIELD"); }
else { sim.applyAction(obp, (int)tx2, (int)ty2, "200"); }
}
sim.nextTurn();
for (int k = 0; k < 4; k++) {
