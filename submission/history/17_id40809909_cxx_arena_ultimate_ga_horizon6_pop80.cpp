...
// MCTS probe constants
static constexpr int MCTS_BUDGET_US = 1000;             // 1ms MCTS probe
static constexpr int MCTS_N_ANGLES = 7;
static constexpr int MCTS_N_THRUSTS = 4;
static constexpr int MCTS_N_CANDIDATES = MCTS_N_ANGLES * MCTS_N_THRUSTS;  // 28
...
inline double evaluate(
    ...
    if (rtimeout == 1) {
        ...
        // Blocker assists runner: push runner toward its next CP
        double dx_br = sim[rp].x - sim[bp].x, dy_br = sim[rp].y - sim[bp].y;
        double dist_br = sqrt(dx_br*dx_br + dy_br*dy_br);
        // Reward being close to our runner (sweet spot: 400-1200px behind)
        if (dist_br < 2000) {
            s += (2000 - dist_br) * 2.0;
        } else {
            s -= (dist_br - 2000) * 1.0;  // penalty for being far
        }
        // Reward being BEHIND runner (opposite side from runner's next CP)
        ...
        // Reward blocker velocity toward runner (ram!)
        if (dist_br > 1.0) {
...
// ── MCTS First-Move Probe (round-robin over discretized runner actions) ──
inline int mcts_first_move_probe(
    ...
    while (1) {
        ...
        for (int t = 1; t < H; t++) {
            ...
            probe.rt[t] = raw < 0 ? 0 : (raw > 200 ? 200 : raw);
        }
        for (int t = 0; t < H; t++) {
            double v = rng.ri(-400, 400) / 10.0;
    ...
    for (int k = 0; k < MCTS_TOP_K; k++) {
        ...
        for (int t = 1; t < H; t++) {
            ...
            int raw = rng.ri(-100, 500);
            seed.rt[t] = raw < 0 ? 0 : (raw > 200 ? 200 : raw);
        }
        for (int t = 0; t < H; t++) {
            double v = rng.ri(-400, 400) / 10.0;
...
inline void run_combined_ga(
    ...
    // ── MCTS-guided first-move seeding ──
    ...
    if (ga_budget_us < 30000) ga_budget_us = 30000;
...
class UltimateBot {
 public:
    ...
    std::pair<BotAction, BotAction> get_actions(
        ...
        // Stuck detection + blocker assist on final lap
        int rtimeout = 0;
        if (runner_stuck_turns_[rp] >= 30) rtimeout = 1;  // blocker races
        // Final lap + stuck: blocker pushes runner toward finish
 ...
 private:
    ...
    int msc_[2] = {};
    bool mbu_[2] = {};
    int runner_stuck_turns_[2] = {};  // turns since last CP crossing per pod
